...
class SvgRenderer:
    ...
    def tostring(self, page: Page) -> bytes:
        return etree.tostring(self.render(page), xml_declaration=True, encoding="UTF-8", pretty_print=True)
    ...
    def _marker(self, line: LineStyle, prefix: str) -> str:
        ...
        etree.SubElement(
            ...
            fill=color,
            **{"fill-opacity": opacity, "stroke": "#000000", "stroke-opacity": "0"},
        )
        return marker_id
    ...
    def _render_shape(self, group: etree._Element, element: Element) -> None:
        ...
        if element.custom_geometry:
            for custom_path in element.custom_geometry:
                path_style = dict(common)
                if not custom_path.fill:
                    path_style.update({"fill": "#000000", "fill-opacity": "0"})
                if not custom_path.stroke:
                    path_style.update({"stroke": "#000000", "stroke-opacity": "0", "stroke-width": "0"})
                etree.SubElement(
                    group,
                    qn(SVG, "path"),
                    d=self._custom_path_d(custom_path, r),
                    **path_style,
                )
            return
        if (
        ...
        if preset == "roundRect":
            etree.SubElement(
                ...
                width=fmt(w),
                height=fmt(h),
                rx=fmt(min(abs(w), abs(h)) * 0.12),
                ry=fmt(min(abs(w), abs(h)) * 0.12),
                **common,
            )
            return
        ...
        if points is not None:
            etree.SubElement(
                group, qn(SVG, "polygon"), points=" ".join(f"{fmt(px)},{fmt(py)}" for px, py in points), **common
            )
            return
    ...
    def _custom_path_d(self, path: GeometryPath, rect: Rect) -> str:
        x = rect.x * self.sx
        y = rect.y * self.sy
        scale_x = rect.width * self.sx / path.width
        scale_y = rect.height * self.sy / path.height

        def point(px: float, py: float) -> str:
            return f"{fmt(x + px * scale_x)} {fmt(y + py * scale_y)}"

        parts: list[str] = []
        for command in path.commands:
            args = command.args
            if command.kind in {"M", "L"}:
                parts.append(f"{command.kind} {point(args[0], args[1])}")
            elif command.kind == "C":
                parts.append("C " + " ".join(point(args[index], args[index + 1]) for index in range(0, 6, 2)))
            elif command.kind == "Q":
                parts.append("Q " + " ".join(point(args[index], args[index + 1]) for index in range(0, 4, 2)))
            elif command.kind == "Z":
                parts.append("Z")
        return " ".join(parts)

    def _image_clip_path(self, element: Element) -> str | None:
        if not element.custom_geometry:
            return None
        ...
        for custom_path in element.custom_geometry:
            etree.SubElement(
                clip,
                qn(SVG, "path"),
                d=self._custom_path_d(custom_path, element.rect),
            )
    ...
    def _preset_points(
        ...
        if preset == "trapezoid":
            return [(x + w * 0.2, y), (x + w * 0.8, y), (x + w, y + h), (x, y + h)]
        if preset == "hexagon":
            return [
                (x + w * 0.25, y),
                (x + w * 0.75, y),
        ...
        if preset == "homePlate":
            return [(x, y), (x + w * 0.75, y), (x + w, y + h / 2), (x + w * 0.75, y + h), (x, y + h)]
        if preset == "corner":
            tx = max(0.005, min(0.5, adjustments.get("adj1", 0.15)))
            ty = max(0.005, min(0.5, adjustments.get("adj2", 0.15)))
    ...
    def _render_text(self, group: etree._Element, element: Element) -> None:
        ...
        for index, run in enumerate(self.text_layouter.layout(element.text, element.rect, self.sx, self.sy), 1):
            ...
            attrs = {
                "x": fmt(run.x),
                "y": fmt(run.baseline_y),
                "fill": run.style.color,
                "fill-opacity": fmt(run.style.opacity),
                "font-family": family,
                "font-size": fmt(run.px_size),
            ...
            if decorations:
                attrs["text-decoration"] = " ".join(decorations)
    ...
    def _render_image(self, group: etree._Element, element: Element) -> None:
        ...
        clip_path = self._image_clip_path(element)
        if clip_path:
            image_attrs["clip-path"] = clip_path
        if element.image_opacity < 1.0:
            image_attrs["opacity"] = fmt(element.image_opacity)
        image = etree.SubElement(
            image_parent,
            qn(SVG, "image"),
            href=element.image_href,
            **{key: value for key, value in image_attrs.items() if key != "href"},
        )
        image.set(qn(XLINK, "href"), element.image_href)
    ...
    def _render_table(self, group: etree._Element, rect: Rect, table: Table, prefix: str) -> None:
        ...
        for row_idx, row in enumerate(table.rows):
            h = (
                table.row_heights_emu[row_idx]
                if row_idx < len(table.row_heights_emu)
                else total_h / max(1, len(table.rows))
            ) * scale_y
            x = rect.x
            col_idx = 0
            for cell_idx, cell in enumerate(row):
                ...
                temp = Element(
                    f"{prefix}-r{row_idx+1}c{cell_idx+1}",
                    "cell",
                    "shape",
                    Rect(x, y, w, h),
                    text=cell.text,
    ...
    def _render_chart(self, group: etree._Element, rect: Rect, chart: Chart, prefix: str) -> None:
        ...
        etree.SubElement(
            ...
            height=fmt(h),
            fill="#FFFFFF",
            **{"fill-opacity": "1", "stroke": "#D0D0D0", "stroke-width": "1"},
        )
        if chart.title:
        ...
        if chart.chart_type in {"pie", "doughnut"}:
            ...
            angle = -math.pi / 2
            cx, cy, radius = plot_x + plot_w / 2, plot_y + plot_h / 2, min(plot_w, plot_h) * 0.38
            for i, value in enumerate(values):
                ...
                if chart.chart_type == "doughnut":
                    ...
                    d = (
                        f"M {fmt(x1)} {fmt(y1)} A {fmt(radius)} {fmt(radius)} "
                        f"0 {large} 1 {fmt(x2)} {fmt(y2)} L {fmt(ix2)} {fmt(iy2)} "
                        f"A {fmt(inner)} {fmt(inner)} 0 {large} 0 {fmt(ix1)} {fmt(iy1)} Z"
                    )
                else:
                    d = (
                        f"M {fmt(cx)} {fmt(cy)} L {fmt(x1)} {fmt(y1)} "
                        f"A {fmt(radius)} {fmt(radius)} 0 {large} 1 {fmt(x2)} {fmt(y2)} Z"
                    )
                etree.SubElement(
                    group,
                    qn(SVG, "path"),
        ...
        minimum = min(0, min(all_values, default=0))
        span = maximum - minimum or 1
        etree.SubElement(
            group,
            qn(SVG, "line"),
            x1=fmt(plot_x),
            y1=fmt(plot_y + plot_h),
            x2=fmt(plot_x + plot_w),
            y2=fmt(plot_y + plot_h),
            stroke="#666666",
        ...
        if chart.chart_type == "line":
            for si, series in enumerate(chart.series):
                ...
                for i, value in enumerate(series.values):
                    px = plot_x + plot_w * i / count
                    py = plot_y + plot_h - (value - minimum) / span * plot_h
                    pts.append((px, py))
                etree.SubElement(
                    group,
                    qn(SVG, "polyline"),
                    points=" ".join(f"{fmt(px)},{fmt(py)}" for px, py in pts),
                    fill="#000000",
                    **{
                        "fill-opacity": "0",
            ...
            category_count = max((len(s.values) for s in chart.series), default=1)
            series_count = max(1, len(chart.series))
            band = plot_w / category_count
            bar_w = band * 0.75 / series_count
            for si, series in enumerate(chart.series):
                for i, value in enumerate(series.values):
                    bh = (value - minimum) / span * plot_h
                    bx = plot_x + i * band + band * 0.125 + si * bar_w
                    by = plot_y + plot_h - bh
                    etree.SubElement(
                        group,
                        qn(SVG, "rect"),
