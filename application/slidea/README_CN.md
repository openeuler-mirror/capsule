...
在 PPT 渲染过程中，会采用 fewshots 的模式，让模型输出的排版保持一致的风格。当前，Slidea 内置了浅色通用，深色通用，红色政治，学术报告，幼小科普5 中模板。Slidea默认会根据用户输入的主题自行选择最合适的模板渲染排版，但用户也可以直接在 PPT 生成任务的请求中，明确要求Slidea采用何种风格的模板。
